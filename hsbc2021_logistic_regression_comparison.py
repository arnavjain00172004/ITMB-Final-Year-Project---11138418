...
Compares two logistic regression models on the exact same sample:
  Model A: HSBC 2021 market features only
  Model B: HSBC 2021 market features + FinBERT & VADER sentiment scores
...
Output files:
    hsbc2021_model_A_market_only_performance.csv
    hsbc2021_model_B_market_sentiment_performance.csv
    hsbc2021_model_A_predictions.csv
    hsbc2021_model_B_predictions.csv
    hsbc2021_coefficients_market_only.csv
    hsbc2021_coefficients_market_sentiment.csv
    hsbc2021_comparison.csv
"""
...
# ─────────────────────────────────────────────────────────────────
# STEP 1 — Load & filter market data to HSBC 2021
# ─────────────────────────────────────────────────────────────────
print("=" * 65)
print("  STEP 1: Loading and filtering market data (HSBC 2021)")
print("=" * 65)
...
print(f"HSBC 2021 rows : {len(hsbc_df)}")
...
# Average multiple headlines on the same date into one daily signal
sent_daily = (
    sent_df
...
# ─────────────────────────────────────────────────────────────────
# STEP 3 — Merge sentiment into HSBC 2021 market rows
# ─────────────────────────────────────────────────────────────────
print("=" * 65)
print("  STEP 3: Merging datasets (backward asof join, 30-day tolerance)")
print("=" * 65)

# Backward asof join: each trading day receives the most recent
# sentiment published on or before that date (within 30 days).
# This handles weekends and days with no news naturally.
...
# Fill any rows before the first headline (truly no signal) with neutrals
...
# Market-only base (drop Date, Stock, Target, and all sentiment cols)
...
# Replace inf -> NaN, then drop rows missing in EITHER feature set
...
# Apply the same indices to the full feature set
...
# ─────────────────────────────────────────────────────────────────
# STEP 7 — Run Model A: Market only
# ─────────────────────────────────────────────────────────────────
print("\n" + "=" * 65)
...
res_A, preds_A, coef_A = run_logistic(
    ...
    "HSBC 2021 — Logistic Regression (Market Only)"
)

# ─────────────────────────────────────────────────────────────────
# STEP 8 — Run Model B: Market + Sentiment
# ─────────────────────────────────────────────────────────────────
print("\n" + "=" * 65)
...
res_B, preds_B, coef_B = run_logistic(
    ...
    "HSBC 2021 — Logistic Regression (Market + Sentiment)"
)
...
compare_df.loc["Δ (B − A)"] = (
    compare_df.loc["HSBC 2021 — Logistic Regression (Market + Sentiment)"]
    - compare_df.loc["HSBC 2021 — Logistic Regression (Market Only)"]
).round(4)
...
# ─────────────────────────────────────────────────────────────────
pd.DataFrame([res_A]).to_csv("hsbc2021_model_A_market_only_performance.csv",    index=False)
pd.DataFrame([res_B]).to_csv("hsbc2021_model_B_market_sentiment_performance.csv", index=False)
preds_A.to_csv("hsbc2021_model_A_predictions.csv", index=False)
preds_B.to_csv("hsbc2021_model_B_predictions.csv", index=False)
coef_A.to_csv("hsbc2021_coefficients_market_only.csv",      index=False)
coef_B.to_csv("hsbc2021_coefficients_market_sentiment.csv", index=False)
compare_df.reset_index().to_csv("hsbc2021_comparison.csv", index=False)

print("\nFiles saved:")
print("  hsbc2021_model_A_market_only_performance.csv")
print("  hsbc2021_model_B_market_sentiment_performance.csv")
print("  hsbc2021_model_A_predictions.csv")
print("  hsbc2021_model_B_predictions.csv")
print("  hsbc2021_coefficients_market_only.csv")
print("  hsbc2021_coefficients_market_sentiment.csv")
print("  hsbc2021_comparison.csv")
